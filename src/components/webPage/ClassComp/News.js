...
//first render gets renders first then didMount gets renderd
    ...
    async componentDidMount(){
        console.log("cdm");
        // let url = ""
        let url = "https://newsapi.org/v2/top-headlines?country=in&apiKey=356f95ea33764cdbade0c4f3f02a9764&page=1&pageSize=20";
...
//please refer classNote.txt for info on increment/decremnt settate
    ...
    increment = async () => {
        if( this.state.page + 1 > Math.ceil(this.state.totalResult/20)){
            ...
            // let url = ""
            let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=356f95ea33764cdbade0c4f3f02a9764&page=${this.state.page + 1}&pageSize=20`;
            let response = await fetch(url);
            let result = await response.json();
            console.log(result)
            this.setState({
        ...
        return(
            <>
                ...
                <div className="d-flex justify-content-between">
                    ...
                    <button className="btn btn-primary" id="next" onClick={this.increment}>Next</button>
                </div>
            </>
        )
    }
